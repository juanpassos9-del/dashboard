...
- **Divisao dos sinais Lightweight:** A pagina `Graficos Avancados` deve renderizar dois componentes Lightweight independentes: um em modo `reversal`, exibindo apenas `REV_BUY`/`REV_SELL`, e outro em modo `trend`, exibindo apenas `TREND_BUY`/`TREND_SELL`. Cada modo deve ter preferencias separadas em `localStorage` para permitir calibragem independente.
