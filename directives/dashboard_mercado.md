...
## Aprendizados
- **Autenticacao:** O dashboard deve bloquear o acesso antes da sidebar quando nao houver usuario autenticado em `st.session_state`. Usar Supabase Auth para cadastro/login com email e senha; cadastro pede apenas email, senha e celular. Salvar celular em `user_metadata` e, quando a tabela `profiles` existir, fazer `upsert` com `user_id`, `email`, `phone`, `is_active` e `updated_at`. Limitar novos cadastros a 1000 usuarios quando a contagem em `profiles` estiver disponivel. Nunca salvar senha em tabela propria. O schema recomendado da tabela fica em `supabase_profiles.sql`.
