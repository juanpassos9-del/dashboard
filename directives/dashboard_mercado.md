...
- **Graficos proprios:** A pagina `Graficos Avancados` pode complementar os widgets TradingView com um componente proprio em `lightweight_chart_component.py`, embutido via `st.components.v1.html`, usando Lightweight Charts v5 (`chart.addSeries(...)`) e dados Binance REST/WebSocket para cripto. Manter os widgets TradingView existentes como fallback ate o modulo proprio estar completamente validado.
