...
- **Fontes gratuitas no Lightweight:** O componente proprio deve aceitar `FINNHUB_API_KEY`/`FINNHUB_TOKEN` para habilitar candles Finnhub e `FRED_API_KEY` para habilitar series macro FRED. As chaves devem ser lidas apenas no backend Streamlit (`st.secrets` ou variaveis de ambiente), nunca expostas no JS. Se a chave nao existir, ocultar os ativos daquela fonte e manter Binance/yfinance funcionando.
