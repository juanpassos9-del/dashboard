...
- **Filtros REV Lightweight:** Para reduzir excesso de sinais, `REV_BUY`/`REV_SELL` devem exigir confluencia entre extremo nas bandas de desvio padrao da VWAP e proximidade/rompimento de bandas HV252 quando o filtro `REV VWAP+HV` estiver ligado. O motor ativo de reversao nao deve usar RSI/IFR como variavel, filtro, score, motivo, tooltip ou controle de painel. Esses filtros sao gates antes do score final, nao apenas criterios visuais.
...
- **Motor REV sem volume como decisao:** O motor ativo de reversao nao deve usar volume bruto, RVOL, media de volume, volume da pernada, expansao de volume, Volume Profile, POC, VAH ou VAL como variavel, filtro, score, motivo, tooltip ou parametro de painel. A reversao deve depender da regiao extrema/confluencia configurada, GARCH quando ligado e candle rejeitando/falhando na regiao. Volume, RVOL, coloracao de candle, Weis Wave e Volume Profile permanecem apenas como leitura visual.
...
- **TTS GARCH(1,1) + HV252 - Modelo Hull:** O grafico proprio deve ter uma camada `GARCH` baseada em GARCH(1,1) sem dependencias externas, com `omega=0.000001`, `alpha=0.08`, `beta=0.90` e validacao `alpha + beta < 1`. Plotar zonas horizontais lognormais/Hull em torno da referencia por 0, 0.5, 1, 1.5, 2, 2.5 e 3 sigma usando `referencia * exp(sigma * multiplicador)`, mostrar no painel volatilidade atual, preco de referencia, modo de referencia e distancia em sigma por log-retorno. A referencia deve poder usar fechamento anterior, abertura da sessao, VWAP, ultimo fechamento ou ajuste manual. Quando `GARCH` e `Filtro GARCH` estiverem ligados, o motor REV deve permitir compra apenas abaixo de `-minSigmaForSignal` e venda apenas acima de `+minSigmaForSignal`, com padrao `1.5`.
