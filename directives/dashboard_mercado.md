...
- **GARCH no Lightweight:** O grafico proprio deve ter uma camada `GARCH` baseada em GARCH(1,1) sem dependencias externas, com `omega=0.000001`, `alpha=0.08`, `beta=0.90` e validacao `alpha + beta < 1`. Plotar zonas horizontais em torno da referencia por 0, 0.5, 1, 1.5, 2, 2.5 e 3 sigma, mostrar no painel volatilidade atual/anualizada, preco de referencia e distancia em sigma. Quando `GARCH` e `Filtro GARCH` estiverem ligados, o motor REV deve permitir compra apenas abaixo de `-minSigmaForSignal` e venda apenas acima de `+minSigmaForSignal`.
