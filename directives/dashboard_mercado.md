...
- **Calendario economico com Atual:** O endpoint Faireconomy/ForexFactory (`ff_calendar_thisweek.json`) normalmente nao entrega o campo `actual`; usar Investing.com como fonte primaria para `Atual/Projecao/Anterior` nos scripts de atualizacao e manter Faireconomy como fallback quando houver bloqueio/rate limit. Nao chamar Investing.com durante a renderizacao do Streamlit Cloud; usar o snapshot do Supabase para evitar tela preta/travamento.
