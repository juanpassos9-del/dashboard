...
- **Ativos Lightweight Brasil/FX:** O grafico proprio em `Graficos Avancados` deve incluir ativos Brasil via `yfinance` (`.SA`, ETFs e `^BVSP`) e referencias BCB gratuitas via SGS para cambio BRL e series macro. Ativos BCB sao diarios e devem forcar timeframes `1d`, `1w` ou `1month`, sem WebSocket.
