...
- **Graficos proprios:** A pagina `Graficos Avancados` pode complementar os widgets TradingView com um componente proprio em `lightweight_chart_component.py`, embutido via `st.components.v1.html`, usando Lightweight Charts v5 (`chart.addSeries(...)`), dados Binance REST/WebSocket para cripto e proxies yfinance pre-carregados para mercado americano/macro. Para yfinance, usar 1m como base, agregar localmente 5m/15m e avisar que 15s/30s usam a base de 1m por falta de granularidade. Manter os widgets TradingView existentes como fallback ate o modulo proprio estar completamente validado.
