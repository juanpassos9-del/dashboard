...
- **Barra fixa Terminal Global:** O Terminal Global deve ter uma barra compacta fixa no lado direito, renderizada apenas em `pagina_terminal_global()`, com atalhos para topo, graficos, calendario, IA Macro e historico. A barra deve reservar padding lateral no desktop e ficar oculta em telas menores para nao sobrepor graficos ou widgets.
- **Graficos Terminal Global:** A aba Terminal Global deve exibir quatro graficos TradingView em duas linhas, dois lado a lado em cada linha, todos com seletores independentes de ativo/intervalo, volume oculto e VWAP diaria/semanal/mensal.
