...
- **Ativos Lightweight Brasil/FX:** O grafico proprio em `Graficos Avancados` deve incluir ativos Brasil via `yfinance` (`.SA`, ETFs e `^BVSP`) e referencias BCB gratuitas via SGS apenas para cambio BRL. Ativos BCB sao diarios e devem forcar timeframes `1d`, `1w` ou `1month`, sem WebSocket. Nao exibir Selic, CDI ou IPCA no seletor do Lightweight.
