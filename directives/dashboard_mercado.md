...
- **Fontes gratuitas no Lightweight:** O componente proprio deve aceitar `FINNHUB_API_KEY`/`FINNHUB_TOKEN` para habilitar candles Finnhub e `FRED_API_KEY` para habilitar series macro FRED. As chaves devem ser lidas apenas no backend Streamlit (`st.secrets` ou variaveis de ambiente), inclusive em formato direto ou aninhado (`[finnhub] api_key`, `[fred] api_key`), nunca expostas no JS. Se Finnhub nao devolver intraday/diario, usar yfinance como fallback para os mesmos tickers quando possivel. Se FRED for selecionado em timeframe intraday, ajustar automaticamente para `1d`, porque FRED entrega series macro diarias.
- **Painel operacional Lightweight:** O componente proprio deve manter crosshair com leitura OHLC/volume/variação/distancia da VWAP, linhas de abertura/maxima/minima/fechamento anterior/preco atual, VWAP diaria/semanal/mensal, bandas percentuais e por desvio, medias SMA/EMA configuraveis, volume relativo, Volume Profile da sessao com POC, VAH e VAL destacados, HV252 calculada com retornos logaritmicos de candles diarios e bandas intradiarias 1/1.5/2 sigma projetadas a partir do fechamento anterior, oscilador de distancia da VWAP, markers BUY/SELL VWAP, alertas laterais, acoes de zoom/export/reload e preferencias em `localStorage`.
