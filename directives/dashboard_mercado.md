...
- **REFS no Lightweight:** A camada `Refs` deve marcar maxima/minima do dia atual, dia anterior, semana anterior e mes anterior, com labels curtos no eixo (`DIA MAX`, `DIA ANT MAX`, `SEM ANT MAX`, `MES ANT MAX` etc.), cores diferentes por grupo e estilos de linha distintos para leitura rapida.
- **Motor de sinais Lightweight:** Os markers do grafico proprio devem ser gerados por motor deterministico local, sem LLM e sem execucao de ordens. O motor deve classificar regime (`uptrend`, `downtrend`, `range`, `stretched_up`, `stretched_down`), pontuar sinais `REV_BUY`, `REV_SELL`, `TREND_BUY` e `TREND_SELL`, aplicar cooldown/score minimo/toggles por tipo e retornar motivos, entrada, stop e alvos. A camada `Sinais` controla a exibicao dos markers, e o crosshair deve mostrar a explicacao do sinal quando houver marker no candle.
- **Filtros REV/TREND Lightweight:** Para reduzir excesso de sinais, `TREND_BUY`/`TREND_SELL` devem exigir confirmacao de pullback por RSI/IFR 14 quando o filtro estiver ligado no painel: compra em zona configuravel de pullback com RSI virando para cima, venda em zona configuravel com RSI virando para baixo. `REV_BUY`/`REV_SELL` devem exigir confluencia entre extremo nas bandas de desvio padrao da VWAP e proximidade/rompimento de bandas HV252 quando o filtro `REV VWAP+HV` estiver ligado. Esses filtros sao gates antes do score final, nao apenas criterios visuais.
...
- **Volume como filtro duro dos sinais:** A validacao de volume do motor REV/TREND deve usar somente volume bruto da barra, media de volume, RVOL, volume da pernada, expansao de volume e pullback com volume fraco. Nao usar volume financeiro nessa etapa. Todo sinal deve exigir RVOL minimo configuravel e volume acima da media; reversao exige extremo + RSI + candle de rejeicao/falha + RVOL e expansao ou volume de pernada; trend exige pullback + RSI + candle de retomada + RVOL e expansao de volume. O painel deve permitir ajustar lookback, RVOL minimo/forte, lookback da pernada e toggles de volume acima da media, expansao e bloqueio de volume caindo.
- **Divisao dos sinais Lightweight:** A pagina `Graficos Avancados` deve renderizar dois componentes Lightweight independentes, um abaixo do outro. Cada grafico deve permitir selecionar as familias `REV` e `TREND` no painel do motor de sinais, alem dos tipos individuais, com preferencias separadas em `localStorage` por instancia.
